...
React UI and backend API on a single toolchain. Set each `CodeFile.language` to
`"typescript"`, `"tsx"` (files with JSX), or `"json"`.
...
lib/                      ← data access, external clients, and pure logic modules (.ts)
...
Add further runtime dependencies (e.g. `zod`, `swr`, a UI library) as the architecture
requires. Pin versions that exist; if a `dep_fix_context` reports an install failure, correct
the offending entry. Set `tsconfig.json` to `"strict": true` with the `@/*` path alias mapped
to the repo root.
